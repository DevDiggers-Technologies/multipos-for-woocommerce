	...
	class DDFW_Form_Field {
		...
		public static function display_form_field( $args ) {
			...
			if ( $args['required'] ) {
				...
				$required        = '&nbsp;<abbr class="required" title="' . esc_attr__( 'required', 'devdiggers-multipos-for-woocommerce' ) . '">*</abbr>';
			} else {
				$required = '';
			}
			...
			switch ( $args['type'] ) {
				case 'country':
					...
					if ( 1 === count( $countries ) ) {
						...
						?>
						<select id="billing_country" name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $args['id'] ); ?>" class="country_to_state country_select <?php echo esc_attr( $default_input_class ); ?> <?php echo esc_attr( implode( ' ', $args['input_class'] ) ); ?>" <?php echo wp_kses_data( implode( ' ', $custom_attributes ) ); ?> data-placeholder="<?php echo esc_attr( $args['placeholder'] ? $args['placeholder'] : esc_attr__( 'Select a country / region&hellip;', 'devdiggers-multipos-for-woocommerce' ) ); ?>" <?php echo wp_kses_post( $data_label ); ?>>
							<option value=""><?php esc_html_e( 'Select a country / region&hellip;', 'devdiggers-multipos-for-woocommerce' ); ?></option>
						...
						<noscript>
							<button type="submit" name="woocommerce_checkout_update_totals" value="<?php esc_attr_e( 'Update country / region', 'devdiggers-multipos-for-woocommerce' ); ?>">
								<?php esc_html_e( 'Update country / region', 'devdiggers-multipos-for-woocommerce' ); ?>
							</button>
						</noscript>
						<?php
					}
							...
							foreach ( $args['options'] as $option_key => $option_text ) {
								if ( '' === $option_key ) {
									if ( empty( $args['placeholder'] ) ) {
										$args['placeholder'] = $option_text ? $option_text : esc_html__( 'Choose an option', 'devdiggers-multipos-for-woocommerce' );
									}
					...
					<div>
						<div class="ddfw-image-upload regular-text">
							<div class="ddfw-image-preview-container <?php echo esc_attr( $value || $args['default_image'] ? '' : 'ddfw-hide' ); ?>">
								<div class="ddfw-image-preview-wrapper">
									...
									<div class="ddfw-image-actions">
										<span class="ddfw-upload-image-button" data-id="<?php echo esc_attr( $args['id'] ); ?>" title="<?php esc_attr_e( 'Replace Image', 'devdiggers-multipos-for-woocommerce' ); ?>">
											<span class="dashicons dashicons-edit"></span>
										</span>
										<span class="ddfw-remove-image-button <?php echo esc_attr( $args['default_image'] && ! $value ? 'ddfw-hide' : '' ); ?>" data-id="<?php echo esc_attr( $args['id'] ); ?>" title="<?php esc_attr_e( 'Remove Image', 'devdiggers-multipos-for-woocommerce' ); ?>">
											<span class="dashicons dashicons-trash"></span>
										</span>
									</div>
								</div>
								<p><?php esc_html_e( 'Hover on the image to replace or remove', 'devdiggers-multipos-for-woocommerce' ); ?></p>
							</div>
							<div class="ddfw-image-upload-wrapper <?php echo esc_attr( $value || $args['default_image'] ? 'ddfw-hide' : '' ); ?>">
								...
								<h3><?php esc_html_e( 'Upload Image', 'devdiggers-multipos-for-woocommerce' ); ?></h3>
								<p><?php esc_html_e( 'Click the upload button to select an image from your media library or upload a new one.', 'devdiggers-multipos-for-woocommerce' ); ?></p>
								...
								<button type="button" class="button ddfw-upload-image-button" data-id="<?php echo esc_attr( $args['id'] ); ?>" <?php echo wp_kses_data( implode( ' ', $custom_attributes ) ); ?>>
									...
									esc_html_e( 'Choose Image', 'devdiggers-multipos-for-woocommerce' );
									?>
								</button>
							</div>
						</div>
				...
				case 'products':
					ob_start();
					?>
					<select name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $args['id'] ); ?>" class="<?php echo esc_attr( $default_input_class ); ?> ddfw-products" <?php echo wp_kses_data( implode( ' ', $custom_attributes ) ); ?> data-placeholder="<?php esc_attr_e( 'Search by name', 'devdiggers-multipos-for-woocommerce' ); ?>">
						<?php
						if ( ! empty( $value ) ) {
							if ( is_array( $value ) ) {
				...
				case 'categories':
					ob_start();
					?>
					<select name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $args['id'] ); ?>" class="<?php echo esc_attr( $default_input_class ); ?> ddfw-categories" <?php echo wp_kses_data( implode( ' ', $custom_attributes ) ); ?> data-placeholder="<?php esc_attr_e( 'Search by name', 'devdiggers-multipos-for-woocommerce' ); ?>">
						<?php
						if ( ! empty( $value ) ) {
							if ( is_array( $value ) ) {
				...
				case 'users':
					ob_start();
					?>
					<select name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $args['id'] ); ?>" class="<?php echo esc_attr( $default_input_class ); ?> ddfw-users" <?php echo wp_kses_data( implode( ' ', $custom_attributes ) ); ?> data-placeholder="<?php esc_attr_e( 'Search Users', 'devdiggers-multipos-for-woocommerce' ); ?>">
						<?php
						if ( ! empty( $value ) ) {
							if ( is_array( $value ) ) {
				...
				case 'user_roles':
					...
					<select
						...
						multiple
						data-placeholder="<?php esc_attr_e( 'Search by role', 'devdiggers-multipos-for-woocommerce' ); ?>"
					>
						<?php
						if ( ! empty( $all_roles ) ) {
