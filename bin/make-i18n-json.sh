...
for T in `find i18n -name "*.pot"`
	do
		sed \
			-e 's/#: src\/pos[^:]*:/#: assets\/js\/pos.js:/gp' \
			-e 's/#: src\/reports[^:]*:/#: assets\/js\/reports.js:/gp' \
			-e 's/#: src\/kitchen[^:]*:/#: assets\/js\/kitchen.js:/gp' \
			$T | uniq > $T-build
		rm $T
		mv $T-build $T
	done

success "Successfully replaced development mappings with production files in devdiggers-multipos-for-woocommerce.pot 🎉 ";
