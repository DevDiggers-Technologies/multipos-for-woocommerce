	...
	foreach ( $args as $key => $arg ) {
		...
		<div class="ddfw-fields-section <?php echo esc_attr( $arg[ 'class' ] ?? '' ); ?>" id="<?php echo esc_attr( $arg[ 'id' ] ?? '' ); ?>">
			...
			if ( ! empty( $arg[ 'after_header_html' ] ) ) {
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Trusted layout HTML supplied by framework configuration.
			...
			if ( ! empty( $arg[ 'submit_button' ] ) ) {
				...
				<p class="submit <?php echo esc_attr( $submit_button[ 'button_parent_class' ] ?? '' ); ?>">
					...
					<button type="submit" id="<?php echo esc_attr( $arg[ 'id' ] ?? '' ) ?>" name="<?php echo esc_attr( $submit_button['name'] ); ?>" class="button button-primary <?php echo esc_attr( $arg[ 'class' ] ?? '' ) ?>" value="<?php echo esc_attr( ! empty( $submit_button[ 'value' ] ? $submit_button[ 'value' ] : __( 'Save', 'devdiggers-multipos-for-woocommerce' ) ) ); ?>">
						<?php
						...
						echo esc_html( ! empty( $submit_button[ 'value' ] ) ? $submit_button[ 'value' ] : __( 'Save', 'devdiggers-multipos-for-woocommerce' ) ); ?>
					</button>
				</p>
				<?php
			}
